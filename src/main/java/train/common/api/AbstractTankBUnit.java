...
public class AbstractTankBUnit extends LiquidTank implements IFluidHandler, INoFuelTransferEntity
{
    public int freightInventorySize;
    private int update = 8;

    public AbstractTankBUnit(World world, Fluid fluid, int quantity, int capacity)
    {
        super(world, fluid, quantity, capacity);
    }

    public AbstractTankBUnit(World world, int capacity)
    {
        super(world, capacity);
    }

    public AbstractTankBUnit(@Nullable FluidStack liquid, int capacity, World world)
    {
        super(liquid, capacity, world);
    }
    ...
    public void onUpdate() {
        ...
        if (getTank() != null && getTank().getFluid() != null) {
            this.dataWatcher.updateObject(18, getTank().getFluidAmount());
            this.dataWatcher.updateObject(4, getTank().getFluid().getFluidID());
            if (getTank().getFluid().amount <= 1) {
                motionX *= 0.94;
                motionZ *= 0.94;
            }
        }
        else if (getTank() != null && getTank().getFluid() == null) {
