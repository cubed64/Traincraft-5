...
public class SlugMA extends LiquidTank implements IFluidHandler, INoFuelTransferEntity
{
	...
	public SlugMA(World world) {
		super(world, EnumTrains.SlugMA.getTankCapacity());
		initFreightWater();
		this.theTank = LiquidManager.getInstance().new FilteredTank(EnumTrains.SlugMA.getTankCapacity(), LiquidManager.dieselFilter());
	}
	...
	public void onUpdate() {
		...
		if (getAmount() > 0) {
			...
			setDefaultMass(-EnumTrains.SlugMA.getMass()*2);
			...
			setDefaultMass(EnumTrains.SlugMA.getMass());
		}
	}
